...
%	\cosupervisor{& John Doe, PhD.} % One co-supervisor.
	...
	The thesis addresses multi-species bird sound classification under low-power embedded constraints. The objective is not to maximize isolated benchmark accuracy, but to design and validate an end-to-end pipeline that remains discriminative under realistic field variability and executable on microcontroller-class hardware.
	...
	This thesis is organized into five chapters, progressing from context to implementation and finally to deployment-oriented conclusions.

	Chapter~1 presents the abstract.
	Chapter~2 introduces the technical and application context. It first frames Edge AI and TinyML constraints, then discusses bioacoustics and conservation technology as the target domain. The chapter also defines the research scope, objectives, and evaluation criteria used throughout the work.
	Chapter~3 reviews the most relevant literature on species monitoring, compact audio models, learnable front-ends, and low-power deployment strategies, and identifies the specific research gap addressed by this thesis.

	Chapter~4 is the core technical chapter. It reports the extended study, including the semi-learnable front-end design, model architecture, dataset construction and preprocessing pipeline, experimental setup, comparative experiments, and result analysis. The same chapter then covers edge deployment on AudioMoth and the device-level benchmarking protocol, so model quality and hardware feasibility are evaluated within one consistent framework.

	Chapter~5 concludes the thesis by summarizing the main findings, discussing current limitations, and outlining future work on model scaling, quantized deployment refinement, and broader field validation.
	...
	AudioMoth and similar devices made large-scale acoustic campaigns economically feasible, but they also exposed practical limits of continuous full-resolution logging \cite{audiomoth2019}. This is why always-on classifiers have become central in low-power ecoacoustics. Existing studies show that lightweight on-device classifiers can run for extended periods while preserving acceptable recognition quality, enabling selective storage and adaptive duty cycles \cite{sabbella2022alwayson,tinychirp2024}. The same direction is visible in broader AI-IoT architectures for bird monitoring, where communication infrastructure and model design are co-optimized for operational continuity \cite{seguragarcia2024}.
	...
	This chapter describes the methods introduced in this thesis.
	This chapter operationalizes the system--algorithm co-design claim by treating representation learning, compact modeling, and deployment profiling as one integrated engineering problem.
	The proposed system combines established building blocks from the literature with original contributions targeted at low-power bioacoustic monitoring. The standard part of the architecture includes STFT-based time-frequency analysis, a compact Matchbox-style convolutional encoder, GRU temporal modeling, and attention pooling. These components are not claimed as novel by themselves; in this thesis they provide a strong, efficient, and reproducible baseline for edge-oriented bird classification.
	...
	\subsubsection{Why This Pipeline Works for Edge Deployment}
	...
	From an edge-computing perspective, this is critical. Small models have limited representational redundancy, so they are sensitive to mismatch between training and deployment audio. By explicitly modeling background diversity, silence structure, and SNR variability during training, the system reduces this mismatch and improves stability in long-duration passive monitoring, where low false-alarm rates and consistent behavior are as important as top-line accuracy.
	...
	The semi-learnable alternative keeps the triangular structure and optimizes only the global transition parameters \(b\) and \(w\). In practical terms, this is a difference between \(2\) trainable front-end parameters and \(32{,}832\). In compact student settings this gap is substantial: moving from a \(\sim 53{,}516\)-parameter baseline to a fully learnable front-end variant raises parameter count to \(\sim 86{,}348\), an increase of about \(61.4\%\). The semi-learnable design avoids this cost while preserving adaptation capacity and interpretability.
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.98\textwidth]{images/model_architecture_blocks.png}
		\caption{Readable block-level view of the proposed model architecture from waveform input to final classification.}
		\label{fig:model_architecture_overview}
	\end{figure}

	\subsubsection{Waveform and Spectral Front-End}
	A waveform is the discrete sequence of air-pressure samples recorded by the microphone. At \(32\)~kHz, a \(3\)-second clip contains \(96{,}000\) samples. This representation is precise in time but does not directly expose spectral structure, while bird calls are primarily discriminated by frequency trajectories, harmonic content, and temporal modulations. For this reason, the first stage of \emph{WrenNet} is a spectral front-end.

	In implementation, the model first enforces a consistent input layout and then applies one of several spectrogram families. In the strongest configuration used in the experiments, the front-end is the semi-learnable combined log--linear module. The waveform is transformed by STFT with Hann windowing, producing complex coefficients over time-frequency bins. Magnitude is extracted and projected through a triangular filterbank with \(N\) filters, where the filter centers are generated by a differentiable mixture of logarithmic and linear spacing. If \(x_i\in[0,1]\) is the normalized filter index, the center frequency is
	\begin{equation}
		f_i = (1-S_i)\,f_{\log}(x_i) + S_i\,f_{\mathrm{lin}}(x_i),
	\end{equation}
	\begin{equation}
		S_i = \sigma\!\left(\left(x_i-\hat{b}\right)w\right), \qquad \hat{b}=\frac{b-f_{\min}}{f_{\max}-f_{\min}},
	\end{equation}
	where \(b\) is the learnable breakpoint and \(w\) is the learnable transition width. The filtered spectrogram is then
	\begin{equation}
		Z_m(t)=\sum_k H_m(k;b,w)\,|X_t(k)|.
	\end{equation}

	After filtering, the model applies decibel conversion with bounded dynamic range and then per-clip normalization:
	\begin{equation}
		\tilde{Z}=\frac{Z_{\mathrm{dB}}-\mu}{\sigma+10^{-5}},
	\end{equation}
	with \(\mu\) and \(\sigma\) computed on the current sample. This normalization stabilizes optimization by reducing scale variability across recordings. The implementation also includes a frequency-axis alignment safeguard that resamples spectrogram channels to the expected number of front-end features when needed, so the downstream encoder receives a consistent tensor geometry.
	...
	\end{equation}
	In practical terms, this stage compresses a long temporal sequence into one discriminative embedding while preserving frame importance through \(\alpha_t\). It is one of the key reasons the architecture remains compact without discarding temporal selectivity.
	...
	\subsubsection{Knowledge Distillation as an Architectural Training Strategy}
	In this project, knowledge distillation is not used as a late-stage add-on, but as a primary training objective for the compact architecture described above. The idea is that a large teacher model provides soft class distributions that encode inter-class similarity and uncertainty structure, while ground-truth labels provide strict supervision on the target class. This combined signal is particularly valuable for bird bioacoustics, where many species are acoustically close and hard labels alone are often insufficient for stable separation \cite{hinton2015distilling,birdnet2021}.

	The teacher used in the distillation pipeline is BirdNET, while the student is the compact \emph{WrenNet} architecture optimized for edge deployment. The implementation follows a two-stage process. First, the teacher processes the training audio to produce soft probability targets with confidence threshold \(0.05\). Second, the student is trained with both hard and soft supervision. The key point is architectural consistency: the deployed student network is unchanged at inference time; distillation only modifies training dynamics and therefore improves accuracy without increasing deployed model complexity.

	The practical objective used in the project is
	\begin{equation}
		\mathcal{L}_{\mathrm{distill}} = \alpha\,\mathcal{L}_{\mathrm{hard}} + (1-\alpha)\,T^2\,\mathcal{L}_{\mathrm{soft}},
	\end{equation}
	with \(\alpha=0.4\) and \(T=3.0\) in the main distilled configuration. Here \(\mathcal{L}_{\mathrm{hard}}\) is standard cross-entropy on ground-truth labels, while \(\mathcal{L}_{\mathrm{soft}}\) is the KL-based soft-target term described in Section~3.5.

	Implementing this pipeline required solving three concrete issues. The first was efficient integration of teacher inference over large training sets; this was addressed by decoupling soft-target generation from student optimization through an offline stage. The second was supervision balance; this was handled through explicit tuning of \(\alpha\), with adaptive adjustment rate \(0.1\) in the adaptive setup. The third was calibration of teacher confidence; temperature scaling with \(T=3.0\) was used to avoid over-peaked targets and provide smoother class-relation signals to the student.

	This distillation design is directly connected to the choice of focal hard supervision discussed below. In practical terms, the training signal is split into two complementary roles: the hard branch must force corrections on difficult and under-represented cases, while the soft branch must transfer inter-class geometry learned by the teacher. The focal term addresses the first role by amplifying hard-example gradients, with \(\gamma=4.0\) in the main protocol and automatic class weighting to handle imbalance. The KL soft term addresses the second role by shaping the student's probability mass over confusable classes. Their combination is therefore coherent with the full architecture: front-end filtering and temporal modeling produce discriminative embeddings, and focal distillation determines how those embeddings are optimized under class imbalance and acoustic ambiguity.

	The measured impact is significant in the distilled nine-class benchmark: the compact student reaches \(88.37\%\) test accuracy while preserving edge-oriented footprint. Gains are especially relevant on challenging species discrimination, with strong improvements in difficult classes such as \emph{Poecile montanus}, and with high recall for the negative ``no bird'' class. These results confirm the intended role of distillation in the proposed system: transfer large-model knowledge into a deployable architecture without sacrificing on-device efficiency.
	...
	In this application context, focal loss is not an isolated mathematical variant. It is aligned with the data reality of bird-sound classification, where difficulty is heterogeneous across species, recordings include strong background variability, and the negative class must be separated from many non-bird acoustic patterns. Under these conditions, plain cross-entropy tends to be dominated by abundant easy examples, whereas focal loss preserves gradient pressure on ambiguous species boundaries and low-SNR segments that are critical for field performance.
	In the main training protocol, focal loss is instantiated with \(\gamma=4.0\) and automatic class weighting, so hard and minority cases maintain non-negligible gradient contribution even when easy examples dominate the batch.
	...
	For consistency with the main distilled configuration discussed in the architecture section, this objective can also be written as \(\mathcal{L}_{\mathrm{distill}}=\alpha\mathcal{L}_{\mathrm{hard}}+(1-\alpha)T^2\mathcal{L}_{\mathrm{soft}}\), with \(\alpha=0.4\) and \(T=3.0\). This parameterization emphasizes soft teacher guidance while retaining hard-label anchoring.
	...
	\subsection{Interpretation of the Semi-Learnable Front-End Behavior}
	...
	This section documents how experiments are executed and compared. The objective is methodological transparency: each result in the following results subsections must be traceable to a clear training, validation, and test procedure.
	...
	The core distillation protocol uses BirdNET soft targets extracted with confidence threshold \(0.05\). The student receives hard labels and soft labels jointly through focal distillation, with \(\alpha=0.4\), \(T=3.0\), and focal parameter \(\gamma=4.0\). Automatic class weighting is enabled for imbalance management, and adaptive distillation uses adaptation rate \(0.1\) to update soft-supervision strength when validation dynamics plateau.
	...
	\begin{table}[t]
		\centering
		\small
		\caption{Key results summary across classification quality and deployment efficiency.}
		\label{tab:key_results_summary}
		\begin{tabular}{|l|c|c|c|c|}
			\hline
			Setting & Test Acc (\%) & Parameters & Energy/inf. [J] & Latency/inf. [s] \\
			\hline
			Easy subset, semi-learnable \(8(+1)\) & 90.76 & 57,546 & -- & -- \\
			Hard subset, semi-learnable \(13(+1)\) & 77.47 & 57,546 & -- & -- \\
			Full dataset, semi-learnable \(70(+1)\) & 66.51 & 57,546 & -- & -- \\
			Full dataset, larger model \(70(+1)\) & 70.14 & 136,906 & -- & -- \\
			AudioMoth deployed student (3 s window) & 87.22\(^\ast\) & \(\sim57\)k & 0.0771 & 1.692 \\
			BirdNET benchmark on Raspberry Pi 3 B+ & -- & -- & 2.79 & 0.978 \\
			\hline
		\end{tabular}
	\end{table}

	\noindent\(^\ast\)Accuracy reported for the corresponding compact student configuration on the controlled \(8(+1)\) benchmark.

	This summary makes the practical positioning explicit. BirdNET remains a strong recognition reference, while the deployment comparison shows that the proposed low-power configuration operates at substantially lower energy cost per decision in continuous monitoring mode, well above the \(16\times\) reference threshold and reaching \(36.2\times\) in the reported benchmark setting.
	...
	\begin{table}[t]
		...
		\begin{tabular}{|l|c|c|c|c|}
			...
			\hline
			Semi-learnable subset & 8(+1) & 64,540 & 87.22 & 87.25 \\
	...
	\begin{table}[p]
		...
		\begin{tabular}{|l|c|c|c|c|c|c|c|c|c|c|c|c|c|c|}
			...
			\hline
			Fully learnable (subset) & 8(+1) & 70k & 75 & 84.53 & 83.82 & 83.84 & 84.77 & 83.27 & -- & -- & -- & -- & -- & -- \\
			\hline
	...
	To isolate the effect of front-end parameterization, an additional controlled comparison was performed on the same \(8(+1)\) setup with identical backbone and training pipeline, changing only the filter-bank management strategy.

	\begin{table}[t]
		\centering
		\small
		\caption{Model-complexity comparison for the direct front-end test.}
		\label{tab:semi_vs_fully_complexity}
		\begin{tabular}{|l|c|c|}
			\hline
			Approach & Learnable Filter Parameters & Total Parameters \\
			\hline
			Semi-learnable (combined log--linear) & 2 (\(b, w\)) & \(\sim 53{,}000\) \\
			Fully learnable filter bank & \(32{,}832\) (\(64\times 513\)) & \(\sim 85{,}000\) \\
			\hline
		\end{tabular}
	\end{table}
	...
	\begin{table}[t]
		\centering
		\small
		\caption{Performance comparison in the direct front-end test.}
		\label{tab:semi_vs_fully_metrics}
		\begin{tabular}{|l|c|c|c|}
			\hline
			Metric & Semi-learnable (\%) & Fully learnable (\%) & Difference (\%) \\
			\hline
			Validation Accuracy & \textbf{85.74} & 76.15 & \(-9.59\) \\
			Test Accuracy & \textbf{87.22} & 79.93 & \(-7.29\) \\
			Weighted \(F1\) & \textbf{87.26} & 79.92 & \(-7.34\) \\
			Weighted Precision & \textbf{87.69} & 81.57 & \(-6.12\) \\
			\hline
		\end{tabular}
	\end{table}
	...
	At the time of writing, this fully learnable comparison was trained with a reduced epoch budget due to computational-time constraints. A matched long-run training is ongoing, but in the current results the semi-learnable front-end is the strongest accuracy/complexity trade-off.

	To complement aggregate metrics, row-normalized confusion matrices are reported for the controlled \(8(+1)\) setting, the difficult \(13(+1)\) subset, and the full \(70(+1)\) setting. For the full setting, a genus-level aggregation is used instead of a dense \(71\times71\) matrix to keep the error structure interpretable.
	...
	The three tables make the subset effect explicit: easy species are characterized by stable and distinctive signatures, while difficult subsets are dominated by taxonomic proximity and overlapping temporal-spectral patterns. This confirms that most residual errors are structured confusions rather than random failures and supports the use of subset-aware analysis for model interpretation.
	...
	The detailed benchmarking protocol, raw electrical measurements, and instrumentation figures are reported in Section~\ref{sec:device_benchmarks}. In summary, relative to Raspberry Pi 3B+ student TFLite inference (\(0.1720\)~J per window), the AudioMoth deployment (\(0.0771\)~J) reduces energy per inference by about \(55\%\). Relative to the Raspberry Pi student STFT baseline (\(1.6500\)~J) and the BirdNET baseline (\(2.7900\)~J), the reductions are about \(95.3\%\) and \(97.2\%\), respectively. The BirdNET gap corresponds to a \(36.2\times\) lower energy cost in this setup, i.e., well above the \(16\times\) threshold highlighted in the paper.

\subsection{AudioMoth Deployment}
\subsubsection{Firmware and Library Integration}
	The hardware deployment stage is built around the AudioMoth platform, selected for its low-power profile and field suitability in long-duration passive acoustic monitoring. From an integration perspective, the firmware layer must guarantee deterministic audio acquisition, stable buffering, and reproducible inference timing under constrained compute and memory budgets.

	The deployment workflow is organized as a two-stage model path. A backbone network transforms fixed-size spectral input into compact temporal features, and a streaming recurrent stage consumes these features together with hidden state to produce class logits at each step. To make this executable on microcontroller hardware, the model conversion chain applies post-training integer quantization and exports compact operators and weights for embedded inference.

	At system level, integration is designed around explicit memory partitioning. The firmware reserves a dedicated tensor arena in external SRAM and splits it between backbone and streaming stages, while keeping internal RAM for control flow and buffering. This separation is fundamental because it prevents contention between runtime bookkeeping and tensor buffers, which would otherwise destabilize long unattended recordings.

	A second design decision is functional simplification of the base firmware to recover resources for inference. Non-essential modules for the benchmarked inference mode are disabled to reclaim Flash and RAM budget, and the runtime path focuses on deterministic inference execution and watchdog-safe operation. In practical terms, this means that the deployment target is not treated as a generic recording firmware with optional AI add-on, but as an inference-oriented firmware profile explicitly engineered for embedded classification.
	...
	The two views in Figure~\ref{fig:audiomoth_hardware_views} make explicit the physical constraints under which the deployed pipeline operates. These constraints are directly relevant to algorithm design because model footprint, buffering policy, and runtime scheduling must remain compatible with microcontroller-class resources.
\subsubsection{On-Device Inference Pipeline}
	The on-device inference pipeline processes fixed windows of audio and produces class probabilities for the monitored species set plus the explicit no-bird class. Execution follows a deterministic sequence: acquisition buffer fill, backbone inference, recurrent streaming update with hidden-state carry-over, posterior normalization, thresholding, and decision emission with a bounded number of events per unit time.

	The most relevant implementation detail is state continuity. The recurrent hidden state is reset only at session boundaries and is propagated between consecutive windows during normal operation. This allows temporal accumulation of evidence while keeping per-window computation bounded. In bird bioacoustics, where vocal signatures often depend on short temporal motifs rather than isolated frames, this continuity is critical for stable on-device classification.

	From a numerical perspective, the deployed pipeline preserves the training design logic. Quantized tensors are dequantized only where required by operator composition, and probabilities are computed from stabilized logits before confidence filtering. This ensures that deployment behavior remains consistent with offline validation while respecting integer-oriented execution constraints.

	Operational robustness is enforced by watchdog-aware scheduling and explicit failure checks at each stage of interpreter and bridge initialization. If a sub-stage fails, the system remains in a controlled state instead of entering undefined execution. This is essential for remote deployments, where a silent failure can invalidate long monitoring campaigns.

\subsection{Device Tests and Benchmarking}
\subsubsection{Functional Tests on Device}
	Functional validation was performed as a hardware-in-the-loop protocol on the target microcontroller. The test sequence includes firmware flashing, controlled reset, breakpoint-assisted verification of initialization phases, and runtime checks on inference completion and benchmark status variables. This protocol is designed to validate not only numerical output availability but also correct execution order of the full pipeline.
...
\label{sec:device_benchmarks}
	...
	Memory benchmarking is based on linker-map analysis and runtime arena checks. The deployed firmware places code and constants in internal Flash, keeps control structures in internal RAM, and allocates a dedicated tensor arena in external SRAM. In the measured build, program text occupies approximately \(0x2A204\) bytes, initialized data approximately \(0x620\) bytes, uninitialized data approximately \(0x5680\) bytes, and the tensor arena region is reserved at \(0x20000\) bytes in external SRAM. This memory split is coherent with the design target of preserving internal RAM headroom for control flow while isolating tensor pressure in external memory.
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.82\textwidth]{images/oscilloscope.jpg}
	...
	\begin{figure}[t]
		\centering
		\includegraphics[width=0.88\textwidth]{images/measurement_example.png}
...
\subsubsection{Exported INT8 Inference Evaluation}
	The exported inference stack uses integer quantization to reduce footprint and to align execution with microcontroller arithmetic capabilities. In deployment terms, quantization is not a standalone compression step but part of an end-to-end consistency requirement: converted models, runtime operators, tensor scales, and threshold policy must remain mutually coherent.

	Two compact quantized sub-models are used in the deployed pipeline, one for backbone feature extraction and one for streaming temporal classification. Their combined footprint remains compatible with the external-SRAM arena strategy and with the firmware budget of the target platform. This modular split also supports staged debugging, because each sub-model can be validated independently before full pipeline execution.

	From the benchmark perspective, the exported INT8 path is considered valid when three conditions hold simultaneously: stable initialization on hardware, non-zero repeated inference timing under sustained loops, and numerically plausible post-softmax confidence distributions with thresholded detections. In the tested configuration, these criteria are satisfied, supporting the claim that the compact model is not only trainable and accurate offline, but also executable and measurable on real edge hardware.

	\chapter{Conclusions and Future Work}
	\section{Conclusions}
	The presented system demonstrates that multi-species bird sound classification can be implemented with a compact architecture while retaining strong practical accuracy. The combination of a semi-learnable spectral front-end, causal convolutional encoding, recurrent temporal memory, and focal-distillation training provides a robust solution for acoustically heterogeneous field data.

	Across experiments, the semi-learnable combined log--linear front-end consistently provides the best trade-off between flexibility and stability. It outperforms fixed mel and linear-triangular mappings in controlled comparisons, and it also outperforms a fully learnable front-end despite using a far more constrained parameterization. This result is central because it shows that structured adaptability can be more effective than unconstrained flexibility in low-footprint bioacoustic models.

	Results on the full \(70(+1)\)-class setting confirm that the approach scales beyond small toy subsets. While larger models recover additional absolute accuracy, the compact student remains competitive and preserves the deployment-oriented profile required for long-duration acoustic monitoring. The measured efficiency gap versus heavier baselines further supports this direction, with order-of-magnitude reductions in inference energy cost on comparable hardware classes and a BirdNET comparison that remains well above the \(16\times\) reference level.

	Overall, the study supports a clear methodological conclusion: effective low-power bioacoustic classification is obtained through co-design of data pipeline, spectral representation, temporal architecture, and training objective, rather than through isolated optimization of a single component.

	From an operational perspective, the measured energy profile has direct implications for campaign scale. Under continuous 3-second windowing, the deployed system operates at approximately \(0.617\)~Wh/day, whereas the BirdNET reference benchmark is approximately \(22.3\)~Wh/day in the same normalization. Under equal daily energy budget, one BirdNET-level node therefore corresponds to about 36 low-power deployed nodes, with clear impact on battery replacement intervals, storage planning, and number of simultaneously monitored locations.
	...
	\section{Future Work}
	A primary development direction is the \emph{Wren Framework}, currently under active implementation. The objective is to turn the methodology presented in this thesis into a reusable standard workflow that can connect domain experts in biology, bioacoustics, and conservation practice with machine-learning pipelines without requiring advanced ML engineering expertise for every new study.
