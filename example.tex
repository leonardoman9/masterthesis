...
%	\cosupervisor{& John Doe, PhD.} % One co-supervisor.
	...
	The reviewed literature shows a clear progression: from isolated recognition models to integrated ecological monitoring systems that must remain accurate, efficient, and robust under field variability. Existing studies provide strong foundations in three directions: large-scale bird recognition, low-power deployment, and adaptive front-end design. At the same time, a concrete gap remains at their intersection. Many high-accuracy models are too heavy for always-on edge operation; many tiny models lose discriminative power on difficult species groups; many front-end studies are validated outside strict low-power multi-species field scenarios.

	This gap motivates the present contribution: a compact bird classification pipeline that explicitly co-designs representation learning, architecture efficiency, and edge deployment constraints, with special focus on semi-learnable front-end behavior in realistic multi-species monitoring conditions.
	...
	This chapter presents the complete technical study behind the proposed system, from the problem definition to the implementation choices and the experimental evidence.
	\section{Extended ICASSP Study}
	...
	Let \(B\) be batch size, \(L\) waveform length, and \(C\) number of classes. For a \(3\)-second clip at \(32\)~kHz, \(L=96{,}000\). The forward path can be written as
	\begin{equation}
		x \in \mathrm{R}^{B\times L}
		\rightarrow S \in \mathrm{R}^{B\times F\times T}
		\rightarrow H \in \mathrm{R}^{B\times F_c\times T}
		\rightarrow R \in \mathrm{R}^{B\times T\times H_d}
		\rightarrow c \in \mathrm{R}^{B\times H_d}
		\rightarrow o \in \mathrm{R}^{B\times C}.
	\end{equation}
	In the compact configuration, \(F=64\), \(F_c=32\), \(H_d=64\). The time length \(T\) is determined by STFT windowing and hop size. This decomposition clarifies that only the final classifier depends directly on \(C\), while front-end, CNN, recurrent, and attention layers remain class-agnostic and therefore reusable across class-set variants.
	...
	To make this interpretation auditable, Figure~\ref{fig:learned_frequency_mappings_16khz} provides a static reconstruction of the interactive ``Learned Frequency Mappings'' report. The plot compares reference mappings and learned mappings derived from the recorded \((b,w)\) values of representative runs.

	\begin{figure}[t]
		\centering
		\includegraphics[width=0.96\textwidth]{images/learned_frequency_mappings_16khz.png}
		\caption{Reconstructed learned frequency mappings from run-level breakpoint and transition-width values (\(0\)--\(16\) kHz). Curves are generated from the same adaptive mapping equations used in the interactive HTML report.}
	...
	\begin{table}[t]
		...
		\begin{tabular}{|l|c|c|c|c|}
			...
			Full dataset compact model & 70(+1) & 57,546 & 66.41 & 67.49 \\
			Full dataset larger model & 70(+1) & 136,906 & 70.13 & 70.81 \\
			\hline
		\end{tabular}
	\end{table}
